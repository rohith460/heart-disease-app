...
def show():
    if st.button("⬅ Back to Awareness"):
        st.session_state.current_page = "awareness"
        st.rerun()
    col1, col2 = st.columns([8, 1])
    st.title("Heart Disease Risk Prediction")

    # 🔴 PASTE YOUR FULL EXISTING CODE BELOW
    ...
    if st.button("🔍 Predict Risk"):
        ...
        **Disclaimer:** This tool is for educational purposes only.  
        ...
        """)
    # =========================================
    # 📂 Batch Prediction using CSV / Excel
    # =========================================
    st.markdown("""
    <style>
    /* File uploader text + box visibility */
    ...
    st.info(
        ...
        "Each row will be predicted individually."
    )
    ...
    if uploaded_file is not None:
        try:
            ...
            if missing_cols:
                ...
                # Predict
                # ---------------------------
                probs = model.predict_proba(df[required_features])[:, 1] * 100

                df["Risk Probability (%)"] = probs.round(2)
                ...
                df["Risk Category"] = df["Risk Probability (%)"].apply(risk_level)
